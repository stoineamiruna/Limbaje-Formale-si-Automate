'''
Numele: Stoinea Maria-Miruna
Grupa: 142
Mail: maria-miruna.stoinea@s.unibuc.ro
...
def recursiv_CFG3(d,prop,index):
    ls=[]
    ok=0
    for i in range(index,len(d["Rules"])):
        ...
        if s in prop:
            ...
            var=recursiv_CFG3(d, prop_nou,i+1)
            if len(var)>1:
                ls+=var
            else:
                ls.extend(var)
...
def CFG3(fisier,prop):
    if load_sections(fisier) == [] or load_sigma(fisier) == [] or load_vars(fisier) == [] or load_rules(fisier) == []:
        return -1
    d = task(fisier)
    return recursiv_CFG3(d,prop,0)
...
#varianta 3, care returneaza o lista cu toate stringurile generate de o anumita gramatica
print(CFG3("exemplu0","A"))
print(CFG3("exemplu1","0A1"))
print(CFG3("exemplu2","EXPR"))
print(CFG3("exemplu3","S"))
print(CFG3("exemplu4","COMMAND"))